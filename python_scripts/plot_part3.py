...
def plot_gantt_chart(ax_memcached, ax_vm4, ax_vm8, tasks, colors):
    ...
    ax_vm4.set_ylabel('     4coreVM')
    ax_memcached.set_ylabel('2coreVM  ')

    ax_vm8.set_ylabel('8coreVM')
...
def main(i):
    ...
    bar_widths = np.diff(start_time)
    bar_widths = np.append(bar_widths, bar_widths[-1])  # Extend the last width for the final bar

    # Plot the p95 latency vs time as bars with edge color
    for i in range(len(start_time)):
        ax1.bar(start_time[i], p95[i], width=bar_widths[i], align='edge', label='p95 latency vs time', color="orange", edgecolor='black')
    ...
    # Plot the QPS vs time as a line
    ax2 = ax1.twinx()
    ...
    tasks_handles.append(memcached_legend)

    plt.suptitle('p95 latency, achieved QPS and job scheduling vs time', y=0.95)
    handle1 = ax1.bar(start_time, p95, width=bar_widths, align='edge', color="orange", edgecolor='black')
